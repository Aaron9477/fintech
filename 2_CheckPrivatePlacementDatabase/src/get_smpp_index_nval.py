...
class Pvn_indexes_profile(Base):
    """
    私募排排，公司规模分布情况
    """
    ...
    index_name = Column(VARCHAR(50))
    index_short_name = Column(Integer)
    index_code = Column(VARCHAR(128))
    isvalid = Column(Integer)
    createtime = Column(DateTime)
    updatetime = Column(DateTime)


class Pvn_pvn_rz_index(Base):
    """
    私募排排，公司规模分布情况
    """
    ...
    index_value = Column(Numeric(22, 6))
...
class GetBaseData(object):
    ...
    def get_all_index(self):
        # 通过ORM操作数据库
        fund_nav = self.session.query(Pvn_indexes_profile).filter(Pvn_indexes_profile.isvalid == 1)
        dt = pd.read_sql(fund_nav.statement, self.engine)
        return dt

    def get_pvn_rz_index(self, index_code):
        # 通过ORM操作数据库
        # 排排网·期货及衍生品策略指数        PPWMFDI
        fund_nav = self.session.query(Pvn_pvn_rz_index).filter(Pvn_pvn_rz_index.index_code == index_code,
                                                                               Pvn_pvn_rz_index.isvalid == 1)
        dt = pd.read_sql(fund_nav.statement, self.engine)
        return dt
...
def get_data(fund_id):
    ...
    fund_nav = base_data_obj.get_pvn_rz_index(fund_id)
    return fund_nav
...
def get_all_index():
    ...
    fund_nav = base_data_obj.get_all_index()
    return fund_nav
...
if __name__ == '__main__':
    ...
    print(updata_data)
    updata_data.to_excel('排排网·期货及衍生品策略指数.xlsx')
