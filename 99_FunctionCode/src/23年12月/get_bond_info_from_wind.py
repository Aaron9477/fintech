...
if __name__ == '__main__':
    ...
    parser.add_argument('--statistics_date', type=str, help='statistics_date', default='2023-12-29')
    parser.add_argument('--output_file', type=str, help='output_file', default='债券信息_23年12月.xlsx')
    ...
    bond_df = df[~df['类别'].isin(['中长期纯债型基金', '可转换债券型基金', '优先股', '混合债券型二级基', '被动指数型债券基',
                                  '货币市场型基金'])]
    bond_code_list = bond_df['债券代码']
    bond_code_list = [x.replace(' ', '').strip() for x in bond_code_list]
