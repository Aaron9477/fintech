...
def get_major_assets2(row, reflact_dict):
    if row['AssetName'] not in reflact_dict.keys():
        return 'None'
    major_assets_detail = reflact_dict[row['AssetName']]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--jy_reflect_file', type=str, help='jy_reflect_file', default='../../data_pybz/大类资产映射划分_jy_230627.xlsx')
    parser.add_argument('--statistics_date', type=str, help='statistics_date', default='2023-06-30')
    args = parser.parse_args()

    jy_reflect_file = args.jy_reflect_file
    statistics_date = args.statistics_date

    jy_raw_files = get_raw_files_jy(statistics_date)
    all_data_file = jy_raw_files[0]
    jy_raw_asset_file = jy_raw_files[1]

    raw_asset_data = pd.read_csv(jy_raw_asset_file)
    df = pd.read_excel(jy_reflect_file, sheet_name='原始科目对应详细大类资产')
    ...
    raw_asset_data['大类资产'] = raw_asset_data.apply(lambda x: get_major_assets2(x, reflact_dict), axis=1)
    raw_asset_data['详细大类资产'] = raw_asset_data.apply(lambda x: get_major_assets_detail(x, reflact_dict), axis=1)

    raw_asset_data.to_excel('jy_金融产品资产配置表映射后.xlsx')
