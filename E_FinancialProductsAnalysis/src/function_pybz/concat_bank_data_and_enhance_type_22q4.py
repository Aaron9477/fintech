...
def get_final_enhance_type(row):
    ...
    if isinstance(row['enhance_type_asset_22q4'], str) and row['enhance_type_asset_22q4'] != '底层数据未披露':
...
def get_final_equity(row):
    # 从23q1到22q3先依次找非空
    if not np.isnan(row['equity_22q4']):
...
if __name__ == '__main__':
    ...
    output_file = target_file.split('/')[-1].split('.')[0] + '_增加固收加分类和权益持仓.xlsx'

    target_df.to_excel(output_file)