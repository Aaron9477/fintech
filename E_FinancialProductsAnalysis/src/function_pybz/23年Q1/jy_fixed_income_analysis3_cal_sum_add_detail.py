...
class FixedIncomeDataType(Enum):
    ...
    shangYinOrJiaoYinOrBeiYin = 4   # 上银或者交银或者北银
    MinSheng = 5    # 民生
...
def compare_first_asset_sum_with_total(input_df, proportion_dict, use_data_col_name):
    if len(input_df[(input_df['AssetName'] == '合计')]) > 0:
        ...
        if abs(sum(proportion_dict.values()) - total) > 0.02:
...
def compare_second_asset_sum_with_total(input_df, first_asset_proportion_dict, second_asset_proportion_dict, use_data_col_name):
    ...
    if len(input_df[(input_df['AssetName'] == '合计')]) > 0:
        total = input_df[(input_df['AssetName'] == '合计')][use_data_col_name].iloc[0]
        if abs(sum(test_dict.values()) - total) > 0.02:
            print(input_df['FinProCode'].iloc[0])
            print(test_dict)
...
def cal_first_asset_proportion(input_df, col_name, fixedIncomeDataType, assetManagementDataType):
    ...
    # 针对上银和交银需要单独处理
    elif fixedIncomeDataType == FixedIncomeDataType.shangYinOrJiaoYinOrBeiYin:
        for idx, row in input_df.iterrows():
            if row['大类资产'] == '固定收益类':
                first_asset['固收'] += row[col_name]

    elif fixedIncomeDataType == FixedIncomeDataType.MinSheng:
        for idx, row in input_df.iterrows():
            if row['大类资产'] == '固定收益类' and (row['详细大类资产'] == '固定收益类' or row['详细大类资产'] == '固定收益类:货币类'):
                first_asset['固收'] += row[col_name]
    ...
    if assetManagementDataType == AssetManagementDataType.onlyOne or assetManagementDataType == AssetManagementDataType.aboveOneFundNumOne:
        ...
        first_asset['资管产品'] = input_df[(input_df['大类资产'] == '资管产品')][col_name].sum() - input_df[(input_df['详细大类资产'] == '资管产品:公募基金')][col_name].min()
...
def cal_second_asset_proportion(input_df, col_name, first_asset_proportion_dict, fixedIncomeDataType):
    ...
    if len(input_df[(input_df['详细大类资产'] == '资管产品:公募基金')]) > 0:
        second_asset_dict['资管产品:公募基金'] = input_df[(input_df['详细大类资产'] == '资管产品:公募基金')][col_name].max()
    ...
    # 针对上银和交银需要单独处理
    elif (fixedIncomeDataType == FixedIncomeDataType.shangYinOrJiaoYinOrBeiYin) and \
            len(input_df[(input_df['大类资产'] == '固定收益类') & (input_df['详细大类资产'] == '固定收益类')][col_name]) > 0:
        second_asset_dict['未穿透的固定'] = input_df[(input_df['大类资产'] == '固定收益类') & (input_df['详细大类资产'] == '固定收益类')][col_name].iloc[0]
    elif fixedIncomeDataType == FixedIncomeDataType.MinSheng and \
            len(input_df[(input_df['大类资产'] == '固定收益类') & (input_df['详细大类资产'] == '固定收益类')][col_name]) > 0:
        second_asset_dict['未穿透的固定'] = input_df[(input_df['AssetName'] == '其他符合监管要求的债权类资产')][col_name].sum()
...
def cal_asset_allocation_ratio(input_df):
    ...
    del output_dict['Unnamed: 0'], output_dict['AssetTypeCode'], output_dict['MarketValue'],\
    ...
    if input_df.iloc[0]['AgentName'] == '上银理财有限责任公司' or input_df.iloc[0]['AgentName'] == '交银理财有限责任公司' \
             or input_df.iloc[0]['AgentName'] == '北银理财有限责任公司':
        fixedIncomeDataType = FixedIncomeDataType.shangYinOrJiaoYinOrBeiYin
    elif input_df.iloc[0]['AgentName'] == '民生理财有限责任公司':
        fixedIncomeDataType = FixedIncomeDataType.MinSheng
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--asset_allocation_file', type=str, help='asset_allocation_file', default='jy_金融产品资产配置表映射后.xlsx')
    parser.add_argument('--top10_fix_income_enhance_analysis_file', type=str, help='input_file', default='jy_前十大持仓固收增强分析.xlsx')
    parser.add_argument('--fund_whether_has_equity_file', type=str, help='fund_whether_has_equity_file', default='jy_资产明细是否有含权基金_基于基金持仓.xlsx')
    ...
    top10_fix_income_enhance_analysis_df = pd.read_excel(args.top10_fix_income_enhance_analysis_file)
    ...
    all_data_df = all_data_df.merge(top10_fix_income_enhance_analysis_df[['FinProCode', '前十大_权益类', '前十大_非标资产', '前十大_商品及衍生品', '前十大_QDII']], how='left', on='FinProCode')
    ...
    tmp_output_df.to_excel('jy_固收增强分类结果.xlsx')
