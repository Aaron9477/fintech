...
def preprocess(input_df):
    ...
    input_df = input_df[(input_df['enhance_type'].notnull()) & (input_df['ProductType'] != '子产品') &
...
if __name__ == '__main__':
    ...
    for group_name in list(grouped.groups.keys()):
        ...
        asset_ratio = cal_asset_ratio(company_df, company_asset_sum)
        ...
        for asset in asset_list:
            ...
            asset_res_list.append(tmp_dict)

            # 记录有披露的公司资产规模/资产占比
            # 未穿透的资产包含未披露的公司
            if asset_ratio['资产规模'][asset] > 0 or asset.startswith('未穿透的'):
                asset_sum_dict[asset_name_first][asset_name_second].append(asset_ratio['资产规模'][asset])
            if asset_ratio['资产占比'][asset] > 0 or asset.startswith('未穿透的'):
                asset_ratio_dict[asset_name_first][asset_name_second].append(asset_ratio['资产占比'][asset])
    ...
    for asset_res in asset_res_list:
        asset_name_first = asset_res['资产大类']
        asset_name_second = asset_res['资产细类']
        if asset_name_first in asset_sum_dict.keys() and asset_name_second in asset_sum_dict[asset_name_first].keys():
            asset_res['有披露的公司资产规模均值'] = sum(asset_sum_dict[asset_name_first][asset_name_second]) / len(asset_sum_dict[asset_name_first][asset_name_second])

    # 大类资产补充均值 只补充有披露公司的均值
    for asset_res in asset_res_list:
        asset_name_first = asset_res['资产大类']
        asset_name_second = asset_res['资产细类']
        if asset_name_first in asset_ratio_dict.keys() and asset_name_second in asset_ratio_dict[asset_name_first].keys():
            asset_res['有披露的公司资产占比均值'] = sum(asset_ratio_dict[asset_name_first][asset_name_second]) / len(asset_ratio_dict[asset_name_first][asset_name_second])
    ...
    asset_res_list_df = pd.DataFrame(asset_res_list).sort_values(['公司名称', '资产大类序号', '资产细类序号'])
