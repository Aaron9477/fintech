...
for col, label in zip(["ON RRP规模", "TGA规模", "ON RRP和TGA之和"], ["ON RRP", "TGA", "ON RRP和TGA之和"]):
    output += f"\n【{label}】\n"
    for period, (prev_date, prev_value, latest_value, change, ratio) in results[col].items():
        if prev_date is not None:
            output += (f"{period}: {ratio:+.2f}%；变化量 {change:+,.0f}；"
                       f"（{prev_date.strftime('%Y-%m-%d')} 规模：{prev_value}）\n")
