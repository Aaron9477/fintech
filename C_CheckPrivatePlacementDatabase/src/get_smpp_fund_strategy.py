...
class GetBaseData(object):
    ...
    def get_fund_strategy(self, fund_id):
        fund_strategy = self.session.query(Pvn_fund_strategy).filter(Pvn_fund_strategy.fund_id == fund_id, Pvn_fund_strategy.isvalid == 1)
...
def get_data_tmp(fund_id):
    base_data = BaseData()
    base_data_obj = GetBaseData(base_data)

    fund_strategy_df = base_data_obj.get_fund_strategy(fund_id)

    print(fund_strategy_df)
    ...
    # bond_name_df = bond_name_df[["fund_id", "fund_name", "fund_short_name"]]
    ...
    #
    # begin_date_df = begin_date_df.rename(columns={'cumulative_nav': begin_date})
    # end_date_df = end_date_df.rename(columns={'cumulative_nav': end_date})
    #
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fund_id', type=str, help='fund_id', default='HF00004RQX')
    args = parser.parse_args()
    updata_data = get_data_tmp(args.fund_id)
