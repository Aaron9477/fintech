# -*- coding=utf-8 -*-
# --------------------------------
# @Time      : 2023年7月24日14:58:00
# @Author    : Noah Zhan
# @File      : licai_comp_fixed_shalves
# @Project   : 银行理财代销图鉴
# @Function  ：生成【底层数据_固定收益类货架_理财公司】依赖表
# --------------------------------
import dateutil
import numpy as np
import pandas as pd
...
def licai_comp_fixed_shalves(start_date,end_date,df1,df2,df7,result_type='single'):
    '''
    function:生成【底层数据_固定收益类货架_理财公司】依赖表
    params:
    ...
    return:
        - 底层数据_固定收益类货架_理财公司:【底层数据_固定收益类货架_理财公司】依赖表，用于更新理财图鉴。
    '''
    import warnings
    warnings.filterwarnings("ignore")
    print(" *生成【 底层数据_固定收益类货架_理财公司】依赖表, ",start_date,"到",end_date,"...")
    ...
    while date >= start_date:
        ...
        temp1_1 = pd.concat([df3_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3,4]).agg('mean'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset=['RegistrationCode'])['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp1_1['是否剔除母子关系'] = '否'
        
        temp1_2 = pd.concat([df3_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset=['RegistrationCode'])['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp1_2['是否剔除母子关系'] = '否'
        ...
        temp1_4 = pd.concat([df3_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp1_4['是否剔除母子关系'] = '否'
        ...
        temp1_6 = pd.concat([df3_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2]).agg('mean'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp1_6['是否剔除母子关系'] = '否'
        ...
        temp1_9 = pd.concat([df3_temp_for9.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3,4]).agg('mean'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp1_9['是否剔除母子关系'] = '否'
        
        temp1_10 = pd.concat([df3_temp_for9.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df3_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')]
                            ,axis=1).reset_index()
        ...
        temp2_1 = pd.concat([df4_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3,4]).agg('mean'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset=['RegistrationCode'])['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp2_1['是否剔除母子关系'] = '是'
        
        temp2_2 = pd.concat([df4_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset=['RegistrationCode'])['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp2_2['是否剔除母子关系'] = '是'
        ...
        temp2_4 = pd.concat([df4_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp2_4['是否剔除母子关系'] = '是'
        ...
        temp2_6 = pd.concat([df4_temp.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2]).agg('mean'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp.groupby(['发行机构','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp2_6['是否剔除母子关系'] = '是'
        ...
        temp2_9 = pd.concat([df4_temp_for9.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3,4]).agg('mean'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品','open_type'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')],
                            axis=1).reset_index()
        temp2_9['是否剔除母子关系'] = '是'
        
        temp2_10 = pd.concat([df4_temp_for9.groupby('发行机构').apply(lambda x:x.groupby(['发行机构_copy','MinInvestTimeType','固收增强分类_补充子产品'])['产品登记编码'].apply(func1)).groupby(level=[0,2,3]).agg('mean'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['BenchmarkMin','RegistrationCode']].apply(lambda x:x.drop_duplicates(subset='RegistrationCode')['BenchmarkMin'].mean()).rename('BenchmarkMin'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])['代销机构'].apply(sort_by_frequency_return_top_n_join_with_sign,ascending=False,n=2,sign=','),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','AssetValue']].apply(weighted_avg_2).rename('加权平均年化收益'),
                            df4_temp_for9.groupby(['发行机构','MinInvestTimeType','固收增强分类_补充子产品'])[['RegistrationCode','年化收益','BenchmarkMin']].apply(func11).rename('达标率')]
                            ,axis=1).reset_index()
        ...
        #合并数据
        底层数据_固定收益类货架_理财公司_temp = pd.concat([temp1_1,temp1_2,temp1_3,temp1_4,temp1_5,temp1_6,temp1_7,temp1_8,temp1_9,temp1_10,temp1_11,temp1_12,
                                        temp2_1,temp2_2,temp2_3,temp2_4,temp2_5,temp2_6,temp2_7,temp2_8,temp2_9,temp2_10,temp2_11,temp2_12],axis=0)
        底层数据_固定收益类货架_理财公司_temp['日期'] = date
    ...
    底层数据_固定收益类货架_理财公司.replace('定开型(申赎不同期)','定开型',inplace = True)
    底层数据_固定收益类货架_理财公司.replace('定开型(申赎同期)','定开型',inplace = True)
    底层数据_固定收益类货架_理财公司.replace('固定收益类','固收非现金', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('封闭短期','封闭短期（一年内）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('封闭中期','封闭中期（一年到三年）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('封闭长期','封闭长期（三年以上）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('开放短期','开放短期（一个月内）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('开放短中期','开放短中期（一月到三月）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('开放中期','开放中期（三月到半年）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('开放中长期','开放中长期（半年到一年）', inplace = True)
    底层数据_固定收益类货架_理财公司.replace('开放长期','开放长期（一年以上）', inplace = True)
