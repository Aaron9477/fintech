...
def licai_comp_product_shelves_info(start_date,end_date,df1,df2,df7,result_type='single'):
    ...
    result_type : str, optional
        数据生成的报表类型，取值有'single'(代表单个公司数据)，'licai_sector'(理财公司分版块),'daixiao_sector'(代销公司分版块)，'sector'(理财公司和代销机构都分版块), by default 'single'

    Returns
    -------
    dataframe
        底层数据_代销产品货架_理财公司 依赖表
    """    
    ...
    # 底层数据_代销产品货架_理财公司
    底层数据_代销产品货架_理财公司['匹配字段']=底层数据_代销产品货架_理财公司[['发行机构','MinInvestTimeType','InvestmentType','是否剔除母子关系','open_type']].sum(axis=1)
