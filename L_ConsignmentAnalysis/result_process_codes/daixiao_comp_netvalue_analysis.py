...
# @Time      : 2023年7月25日
# @Author    : WSS
# @File      : daixiao_comp_netvalue_analysis
# @Project   : 银行理财代销图鉴
# @Function  ：生成【底层数据_净值指标_代销机构】依赖表
# --------------------------------
#需要用到的库
import pandas as pd
...
def daixiao_comp_netvalue_analysis(start_date,df1,df2,df7,result_type='single'):
    '''
    function:生成【底层数据_净值指标_代销机构】依赖表
    params:
    ...
    df8_temp = pd.merge(df7,df3_temp,on='FinProCode')
    df8_temp = df8_temp.drop_duplicates(subset=['代销机构','FinProCode'])
    周度时间_list=list(df7.columns[:])
    周度时间_list=周度时间_list+['代销机构','InvestmentType','AssetValue']
    
    for m in ['否','是']:
        if m=='是':
            df8_temp = pd.merge(df7,df4_temp,on='FinProCode')#剔除母子公司之间建立的代销关系
            df8_temp = df8_temp.drop_duplicates(subset=['代销机构','FinProCode'])
        净值指标_加权平均=df8_temp[周度时间_list].groupby(['代销机构','InvestmentType']).apply(np_average_juzhen)
        ...
        净值指标_代销机构=净值指标_代销机构.swaplevel('代销机构','InvestmentType')
    
    #计算排名
        for j in 净值指标_代销机构.index.get_level_values('InvestmentType').unique():
            count_ret=净值指标_代销机构.loc[j,'oneyear_return'].count()
            count_maxdraw=净值指标_代销机构.loc[j,'max_markdown'].count()
            count_sharpo=净值指标_代销机构.loc[j,'sharpo'].count()
            count_asset=净值指标_代销机构.loc[j,'Asset'].count()
            for i in 净值指标_代销机构.loc[j].index:
                净值指标_代销机构.loc[(j,i),'ret_rank']=净值指标_代销机构.loc[j,'oneyear_return'].rank(method='min',ascending=True)[i]/count_ret
                净值指标_代销机构.loc[(j,i),'oneyear_rank']=净值指标_代销机构.loc[j,'oneyear_return'].rank(method='min',ascending=False)[i]
                净值指标_代销机构.loc[(j,i),'oneyear_count']=count_ret
                净值指标_代销机构.loc[(j,i),'maxdraw_rank']=净值指标_代销机构.loc[j,'max_markdown'].rank(method='min',ascending=True)[i]/count_maxdraw
                净值指标_代销机构.loc[(j,i),'sharpo_rank']=净值指标_代销机构.loc[j,'sharpo'].rank(method='min',ascending=True)[i]/count_sharpo
                净值指标_代销机构.loc[(j,i),'asset_rank']=净值指标_代销机构.loc[j,'Asset'].rank(method='min',ascending=True)[i]/count_asset
        净值指标_代销机构[['ret_rank','maxdraw_rank','sharpo_rank','asset_rank']]=净值指标_代销机构[['ret_rank','maxdraw_rank','sharpo_rank','asset_rank']].fillna(0)
        #计算得分
        for j in 净值指标_代销机构.index.get_level_values('InvestmentType').unique():
            for i in 净值指标_代销机构.loc[j].index:
                净值指标_代销机构.loc[(j,i),'score']=cal_score(j,净值指标_代销机构.loc[(j,i),'asset_rank'],净值指标_代销机构.loc[(j,i),'ret_rank'],净值指标_代销机构.loc[(j,i),'maxdraw_rank'],净值指标_代销机构.loc[(j,i),'sharpo_rank'])
        #对得分进行排名     
        for j in 净值指标_代销机构.index.get_level_values('InvestmentType').unique():
            count_score=净值指标_代销机构.loc[j,'score'].count()
            for i in 净值指标_代销机构.loc[j].index:
                净值指标_代销机构.loc[(j,i),'score_rank']=净值指标_代销机构.loc[j,'score'].rank(method='min',ascending=False)[i]
                净值指标_代销机构.loc[(j,i),'score_sum']=count_score
    
        if m=='是':
            净值指标_代销机构['是否剔除母子关系']='是'
            净值指标_代销机构.sort_index(inplace=True)
            底层数据_净值指标_代销机构_是=净值指标_代销机构.copy()
        if m=='否':
            净值指标_代销机构['是否剔除母子关系']='否'
            净值指标_代销机构.sort_index(inplace=True)
            底层数据_净值指标_代销机构_否=净值指标_代销机构.copy()
            
    底层数据_净值指标_代销机构=pd.concat([底层数据_净值指标_代销机构_否,底层数据_净值指标_代销机构_是]) 
    底层数据_净值指标_代销机构=底层数据_净值指标_代销机构.drop(['ret_rank','maxdraw_rank','sharpo_rank','asset_rank'], axis=1)
    底层数据_净值指标_代销机构.replace(np.inf,'-', inplace = True)
    底层数据_净值指标_代销机构=底层数据_净值指标_代销机构.fillna(value="-")
    底层数据_净值指标_代销机构 = 底层数据_净值指标_代销机构.swaplevel('代销机构','InvestmentType')
    return 底层数据_净值指标_代销机构
