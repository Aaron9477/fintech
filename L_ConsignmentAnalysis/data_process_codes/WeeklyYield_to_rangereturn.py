# -*- coding=utf-8 -*-
# --------------------------------
# @Time      : 2023年7月24日10:16:08
# @Author    : Noah Zhan
# @File      : WeeklyYield_to_rangereturn
# @Project   : 银行理财代销图鉴
# @Function  ：根据将七日年化序列数据计算以最新期为截止时间的区间收益
# --------------------------------

import pandas as pd


def WeeklyYield_to_rangereturn(x,length = 13,frequency = 1):
    '''
    function:根据将七日年化序列数据计算以最新期为截止时间的区间收益
    params:
        - x:pd.Series，现金管理类产品的七日年化序列数据;
    ...
    try:
        ...
        # print("error")
        return
    return sum(x.iloc[ind])*(7/365)*0.01