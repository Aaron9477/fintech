...
def cal_first_asset_proportion(input_df, col_name, fixedIncomeDataType, assetManagementDataType):
    ...
    # 针对上银和交银需要单独处理
    elif fixedIncomeDataType == FixedIncomeDataType.shangYinOrJiaoYin:
        for idx, row in input_df.iterrows():
            if row['大类资产'] == '固定收益类':
                first_asset['固收'] += row[col_name]
...
def cal_second_asset_proportion(input_df, col_name, first_asset_proportion_dict, fixedIncomeDataType):
    ...
    # 针对上银和交银需要单独处理
    elif fixedIncomeDataType == FixedIncomeDataType.shangYinOrJiaoYin and len(input_df[(input_df['大类资产'] == '固定收益类') & (input_df['详细大类资产'] == '固定收益类')][col_name]) > 0:
...
def judge_enhancement_type(input_df):
    ...
    if input_df.iloc[0]['AgentName'] == '上银理财有限责任公司' or input_df.iloc[0]['AgentName'] == '交银理财有限责任公司':
        fixedIncomeDataType = FixedIncomeDataType.shangYinOrJiaoYin
    else:
...
def fix_error_of_yunongshang(input_df, yunongshang_df):
    asset_list = ['固收', '资管产品', '混合类', 'QDII', '其他', '权益类', '商品及衍生品', '非标资产', "固定收益类:货币类",
         "固定收益类:债券类", "固定收益类:资产支持证券", "资管产品:公募基金", "资管产品:私募资管产品/信托计划/计划类资产"]
    FinProCode_list = list(yunongshang_df['FinProCode'])
    for FinProCode in FinProCode_list:
        for asset in asset_list:
            input_df.loc[input_df['FinProCode'] == FinProCode, asset] = yunongshang_df.loc[yunongshang_df['FinProCode'] == FinProCode, asset].values[0]
    return input_df

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--input_file', type=str, help='input_file', default='金融产品资产配置表_分类后.xlsx')
    parser.add_argument('--target_file', type=str, help='target_file', default='../data/bank_wealth_product_12_11.csv')
    parser.add_argument('--series_name_file', type=str, help='series_name_file', default='../data/系列名称对应.xlsx')
    parser.add_argument('--top10_file', type=str, help='input_file', default='前十大持仓固收增强分析.xlsx')
    parser.add_argument('--yunongshang_file', type=str, help='yunongshang_file', default='渝农商基于前十大持仓统计大类资产.xlsx')
    args = parser.parse_args()

    df = pd.read_excel(args.input_file)
    top10_df = pd.read_excel(args.top10_file)
    yunongshang_df = pd.read_excel(args.yunongshang_file)
    ...
    # 修复渝农商不公布穿透前数据的问题
    output_df = fix_error_of_yunongshang(output_df, yunongshang_df)
