...
def get_fund_all(conn, cursor, name):
    sql = f"select fund_name, strategy, administrator, region, manager, establish_time from fund_base_info where category='{name}'"
    ...
    # fund_name = fund_name_list.size-1
        ...
        for count in range(fund_num):
          ...
          # fund_name_list.values[0, 1 + count]
            ...
            res_down = pd.concat([res_down, fd_processed_new], axis=1).sort_index()

        index_list = list(res_down.index)
        res_down.index = [x.strftime('%Y/%m/%d') for x in index_list]
        ...
        fund_all_up = fund_all.T


        """整理表的上半部分"""
        res_up= pd.DataFrame()
        for count in range(fund_num):
            res_up = pd.concat([res_up, fund_all_up.loc[:, count]], axis=1)
            res_up = pd.concat([res_up, pd.DataFrame(columns=[count])], axis=1)
            res_up = pd.concat([res_up, pd.DataFrame(columns=[count])], axis=1)

        """将表的上下两部分合在一起"""
        res_down = res_down.T
        res_up = res_up.T
        res_all = pd.concat([res_up.reset_index(drop=True), res_down.reset_index()], axis=1)
        res_all = res_all.reset_index(drop=True).T.reset_index()
        res_all.iat[0, 0] = float('nan')
        res_all.iat[6, 0] = float('nan')
        ...
        if name == '指数增强new':
            ...
            res_zhi = pd.DataFrame()
            res_zhi = pd.concat([res_zhi, res_all.loc[:,'index']], axis = 1)
            for count in range(fund_num):
                count1 =  count*3
                res_zhi = pd.concat([res_zhi, res_all.loc[:, count1]], axis=1)
                res_zhi = pd.concat([res_zhi, res_all.loc[:, count1+1]], axis=1)
                res_zhi = pd.concat([res_zhi, res_all.loc[:, count1+2]], axis=1)
                for k in range(5):
                    res_zhi = pd.concat([res_zhi, pd.DataFrame(columns=[str(count)+'加'+str(k)])], axis=1)
            for count in range(fund_num):
                res_zhi.loc[6, str(count) + '加0'] = '标准化'
                res_zhi.loc[6, str(count) + '加1'] = '超额收益（复利）'
                res_zhi.loc[6, str(count) + '加2'] = '超额收益（单利）'
                res_zhi.loc[6, str(count) + '加3'] = '周度超额收益率'
                res_zhi.loc[6, str(count) + '加4'] = '超额最大回撤'
                res_zhi.loc[0, str(count) + '加1'] = res_zhi.loc[0, count*3]+'超额收益'
            res_all = res_zhi
        res_all.to_excel(writer, sheet_name=name, header=None, index=False, float_format="%.6f")
