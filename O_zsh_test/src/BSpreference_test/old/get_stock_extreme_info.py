    ...
    if max(max(np.append(extres_min,-10**10)),max(np.append(extres_max,-10**10)))<max_point:
        if get_max(extres_min)<get_max(extres_max):
            if array[extres_max[-1]]>array[-1]+0.001:
                extres_min=np.insert(extres_min, -1, max_point) if len(extres_min)>0 else np.insert(extres_min, 0, max_point)
            else:
                extres_max=np.delete(extres_max,-1)
                extres_max=np.insert(extres_max, -1, max_point) if len(extres_max)>0 else np.insert(extres_max, 0, max_point)
    ...
    for stock in stocks:
        ...
        for date in [extres_min_date, extres_max_date]:
            for single_date in date:
                result_list.append({
                    'stock': stock,
                    'extremum_date': single_date,
                    'extremum_value': recent_stock_price.loc[recent_stock_price['backdate'] == single_date, 'close'].values[0],
                    'min_or_max': 'min' if date is extres_min_date else 'max'
                })

    result_df = pd.DataFrame(result_list)

        # # extres_min_date = pd.to_datetime(extres_min_date).strftime('%Y%m%d')
        # # extres_max_date = pd.to_datetime(extres_max_date).strftime('%Y%m%d')
        #
        # # 将结果添加到 result_df 中
        # result_df = result_df.append({
        #     'stock': stock,
        #     'extres_min_date': extres_min_date,
        #     'extres_max_date': extres_max_date
        # }, ignore_index=True)
...
if __name__ == '__main__':
    ...
    stock_extreme_info.to_csv('财汇股票行情数据_240603_极值数据.csv', index=False)