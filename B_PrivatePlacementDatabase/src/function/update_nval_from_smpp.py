...
class GetBaseData(object):
    ...
    def get_fund_nav(self, fund_list, tradeday):
        fund_nav = self.session.query(Pvn_nav_all_business).filter(Pvn_nav_all_business.fund_id.in_(fund_list), Pvn_nav_all_business.isvalid == 1,
                                                      Pvn_nav_all_business.price_date == tradeday)
